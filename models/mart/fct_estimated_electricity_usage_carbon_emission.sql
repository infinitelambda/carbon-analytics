...
company_locations as (
  ...
  CASE
    WHEN fl.LOCATION_TYPE IS NULL THEN 'Office'
    ELSE fl.LOCATION_TYPE
  END AS LOCATION_TYPE_FOR_JOINING --when the client has location types for which we do not have factors, we just use the Office factors
  FROM {{ ref('stg_operational_data__company_locations') }} CD
  LEFT JOIN factored_locations FL on FL.LOCATION_TYPE=cd.LOCATION_TYPE
)
SELECT
...
LEFT JOIN {{ ref('stg_additional_modelling_resources__us_commercial_buildings_demand') }} dem ON upper(dem.TYPE)=upper(cd.LOCATION_TYPE_FOR_JOINING)
...
LEFT JOIN weighted_daily_intensity ac on ac.REGION_NAME=r.CARBON_INTENSITY_REGION and cd.LOCATION_TYPE_FOR_JOINING = ac.LOCATION_TYPE
