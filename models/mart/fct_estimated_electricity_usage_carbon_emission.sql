...
*/
with
--{% jinja that activates UK-only parts if uk_locations = True %}
...
/*
We can start asking clients to provide what region their locations fall in.
Instead of mapping postcodes to carbon intensity regions, stg_additional_modelling_resources__carbon_intensity_regions_mapped
can get changed to have the following data:
 - REGION_NAME - name of area for which we have data. This can be UK/US/China/Canada/Australia carbon intensity region, or a country (e.g. France, Bulgaria, etc)
 - REGION_TYPE - whether it is a carbon intensity region or a country.
*/
...
--)
--{% end of jinja that activates UK-only parts if uk_locations = True %}

--{% jinja that activates the below comma if uk_locations = True and country_data_locations=True %}
--,
--{% end of jinja that activates the comma if uk_locations = True and country_data_locations=True %}

--{% jinja that activates if country_data_locations=True AND/OR region_data_locations=True %}
-- date_spine AS (
...
-- )
--{% end of jinja that activates if country_data_locations=True AND/OR region_data_locations=True %}

--{% jinja that activates if country_data_locations=True %}
...
-- )
--{% end of jinja that activates if country_data_locations=True %}

--{% jinja that activates the below comma if (uk_locations=True AND/OR country_data_locations=True) AND region_data_locations=True %}
--,
--{% end of jinja that activates the comma if (uk_locations=True AND/OR country_data_locations=True) AND region_data_locations=True %}

--{% jinja that activates if region_data_locations=True %}
-- region_level_emissions as (
--[sql query to be added, will be very similar to the one in country_level_emissions CTE]
-- )
--{% end of jinja that activates if region_data_locations=True %}

--{% jinja that activates if uk_locations=True %}
-- SELECT * FROM uk_level_emissions
--{% end of jinja that activates if uk_locations=True %}

--{% jinja that activates a UNION if uk_locations=True and (region_data_locations=True AND/OR country_data_locations=True) %}
-- UNION
--{% end of jinja that activates a UNION if uk_locations=True and (region_data_locations=True AND/OR country_data_locations=True) %}

--{% jinja that activates if country_data_locations=True %}
--SELECT * FROM country_level_emissions
--{% end of jinja that activates if country_data_locations=True %}

--{% jinja that activates a UNION uk_locations, region_data_locations and country_data_locations are True  %}
-- UNION
--{% end of jinja that activates a UNION uk_locations, region_data_locations and country_data_locations are True %}

--{% jinja that activates if region_data_locations=True %}
--SELECT * FROM region_level_emissions
--{% end of jinja that activates if region_data_locations=True %}
